...
public class ProductRepositoryImpl implements ProductRepositoryCustom {
     ...
     */
    // TODO: 가격높은순 구현
    public List<GetProductResponse> findPageByProductPriceDesc(Long cursorId, Long cursorPrice, Long categoryId, int pageSize) {
        ...
        return jpaQueryFactory
                ...
                .leftJoin(productCategory)
                .on(productCategory.product.id.eq(product.id))
                .where(cursorIdAndCursorPrice(cursorId, cursorPrice),
                        categoryIdEq(categoryId),
                        product.productState.eq(ProductState.SELLING))
    ...
    private BooleanExpression cursorIdAndCursorPrice(Long cursorId, Long cursorPrice) { // 첫 페이지 조회와 두번째 이상 페이지 조회를 구분하기 위함
